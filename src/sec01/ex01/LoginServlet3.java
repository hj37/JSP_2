...
//흐름 > 클라이언트가 웹브라우저 주소창에 http: //localhost:8080/pro06/login.html을 입력 후 
// 	정적인 페이지인 login.html을 아파치 웹서버에 요청한다.
// 정적인 login.html페이지가 웹브라우저 화면에 나오면 클라이언트는 아이디, 비밀번호를 입려후 
...
public class LoginServlet3 extends HttpServlet {
	// 메소드 오버라이딩(메소드 기능 재구현)

	// 클라이언트가 login.html에서 이름과 비밀번호 입력후 로그인버튼(전송,요청버튼)을 클릭했을때...
	// 현재 위의 LoginServlet을 요청한다.
	// 요청시.. 입력한 정보들을 HttpServletRequest객체에 저장 후~ doGet 메소드의 매개변수로 전달한다.
	// 이런 HttpServletRequest객체는 톰캣컨테이너가 객체를 생성한 후 doGet메소드의 매개변수로 전달한다.
	@Override
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//request객체에 저장된 요청값들이 한글이 하나라도 존재하면 깨지므로 인코딩(문자셋) 방식을 UTF-8로 셋팅
		...
		//2.요청값을 얻은 후 응답값 마련하기	(예를 들면...DB와 연동하여 회원정보를 검색한 데이터) 
